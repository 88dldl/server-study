...
public class MemberRestController {
    ...
    */
    @Operation(summary = "내가 쓴 리뷰 목록 조회 API",description = "내가 쓴 리뷰 목록 조회을 조회하는 API이며, 페이징을 포함합니다. query String 으로 page 번호를 주세요")
    ...
    public ApiResponse<MemberResponseDTO.ReviewPreViewListDTO> getReviewList(@ExistMember @PathVariable(name = "memberId") Long memberId, @CheckPage @RequestParam(name = "page") Integer page){
        Page<Review> tempPage = memberQueryService.getReviewList(memberId,page);
        return ApiResponse.onSuccess(MemberConverter.reviewPreViewListDTO(tempPage));
    }

    @Operation(summary = "내가 쓴 미션 목록 조회 API",description = "내가 쓴 미션 목록 조회을 조회하는 API이며, 페이징을 포함합니다. query String 으로 page 번호를 주세요")
    ...
    public ApiResponse<MemberResponseDTO.MissionPreViewListDTO> getMissionList(@ExistMember @PathVariable(name = "memberId") Long memberId, @CheckPage @RequestParam(name = "page") Integer page){
        Page<Mission> tempPage = memberQueryService.getMissionList(memberId,page);
        return ApiResponse.onSuccess(MemberConverter.missionPreViewListDTO(tempPage));
    }
